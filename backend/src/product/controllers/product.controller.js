...
export const getAllProducts = async (req, res, next) => {
  try
  {
    ...
    if (keywords.hasOwnProperty("keyword")) {
      ...
      if(keywords.hasOwnProperty("price") && keywords.price.hasOwnProperty("gte"))
      {
        console.log("hel1");

        const pricing = keywords["price"]["gte"];
        filter["price"] = { $gte: parseInt(pricing) }; 
      }
      if(keywords.hasOwnProperty("price") && keywords.price.hasOwnProperty("lte"))
      {
        console.log("hel");
        const price = keywords["price"]["lte"];
       
        filter["price"] = {...filter["price"],$lte: parseInt(price) }; 
      }
      if(keywords.hasOwnProperty("rating") && keywords.rating.hasOwnProperty("gte"))
      {
        console.log("hel1");

        const pricing = keywords["rating"]["gte"];
        filter["price"] = { $gte: parseInt(pricing) }; 
      }
      if(keywords.hasOwnProperty("rating") && keywords.rating.hasOwnProperty("lte"))
      {
        console.log("hel");
        const price = keywords["rating"]["lte"];
       
        filter["rating"] = {...filter["rating"],$lte: parseInt(price) }; 
      }
    console.log(pageId);
    console.log(filter);
    const allProduct = await getAllProductsRepo(filter,pageId);
...
export const deleteReview = async (req, res, next) => {
  ...
  try {
    ...
    console.log(userId);
    console.log(reviewToBeDeleted.user);
    if(reviewToBeDeleted.user.equals(userId))
    {
    ...
    else
    {
      ...
      msg:"you are not Authorized to this request"})
    }
  } catch (error) {
    return next(new ErrorHandler(500, error));
  }
